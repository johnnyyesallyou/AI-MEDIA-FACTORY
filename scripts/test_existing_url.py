...
db = SessionLocal()
try:
    # Берём content с существующим image_url
    ...
    if not content:
        print('❌ Нет content с image_url для теста')
        sys.exit(1)
    
    print(f'Используем content: {content.headline[:50]}')
    ...
    asset = manager.save_from_url(
        ...
        prompt=content.image_prompt or '',
    ...
    if asset:
        ...
        print(f'   extra_data: {asset.extra_data}')
