...
try:
    items = adapter.fetch_latest_chapters(limit=50)
...
count = db.query(ContentORM).filter(ContentORM.status == "research").count()
print(f"\nCreated: {total} new ReManga items")
print(f"Total research items: {count}")
db.close()
print("=" * 70)