...
print("=" * 70)
print("TEST: SmartImageResolver on manga")
print("=" * 70)

db = SessionLocal()
resolver = SmartImageResolver()

# Берём последний manga research item
...
if not item:
    print("No manga items without covers")
    db.close()
    exit(0)
...
print(f"manga_cover_url: {metadata.get('manga_cover_url', 'None')[:60]}")
