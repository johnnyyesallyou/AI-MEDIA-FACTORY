    ...
    def run(self, channel: ChannelORM = None, limit: int = 20) -> Dict[str, Any]:
        ...
        try:
            ...
            if not anime_channel:
                ...
                try:
                    mode = (anime_channel.content_profile or {}).get("publishing_mode", "auto")
                except Exception:
                    mode = "auto"
                if mode != "auto":
                    try:
                        moved = db.query(ContentORM).filter(ContentORM.channel_id == anime_channel.id, ContentORM.status == "research").update({"status": "draft"})
                        db.commit()
                    except Exception:
                        moved = 0
